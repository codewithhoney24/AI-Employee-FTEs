...
The Brain:  Gemini Code acts as the reasoning engine. We add the Ralph Wiggum Stop hook to let the agent continuously iterate until the assigned task is complete.
...
Purpose
 Gemini Code
Active subscription (Pro or Use Free Gemini API with  Gemini Code Router)
...
Able to use and prompt  Gemini Code
Prompt  Gemini Code to to convert AI functionality into Agent Skills

Pre-Hackathon Checklist
Install all required software listed above
Create a new Obsidian vault named "AI_Employee_Vault"
Verify  Gemini Code works by running:  Gemini --version
...
One working Watcher script (Gmail OR file system monitoring)
 Gemini Code successfully reading from and writing to the vault
...
Auto-comment on relevant posts by topic/keyword (LinkedIn Auto-Commenter)
Reply to comments on your posts (LinkedIn Notification Handler)
 Gemini reasoning loop that creates Plan.md files
...
The Muscle ( Gemini Code): Runs in your terminal, pointed at your Obsidian vault. It uses its File System tools to read your tasks and write reports. The Ralph Wiggum loop (a Stop hook) keeps  Gemini iterating until multi-step tasks are complete.

2. Architecture: Perception → Reasoning → Action
A. Perception (The "Watchers")
Since  Gemini Code can't "listen" to the internet 24/7, you use lightweight Python Sentinel Scripts running in the background:
...
The Watcher layer is your AI Employee's sensory system. These lightweight Python scripts run continuously, monitoring various inputs and creating actionable files for  Gemini to process.
...
B. Reasoning ( Gemini Code)
When the Watcher detects a change, it triggers a  Gemini command:
...
Plan:  Gemini creates a Plan.md in Obsidian with checkboxes for the next steps.
C. Action (The "Hands")
Model Context Protocol (MCP) servers are  Gemini Code's hands for interacting with external systems. Each MCP server exposes specific capabilities that  Gemini can invoke.
 Gemini uses custom MCP (Model Context Protocol) servers to act:
...
Human-in-the-Loop (HITL):  Gemini writes a file: APPROVAL_REQUIRED_Payment_Client_A.md. It will not click "Send" until you move that file to the /Approved folder.
...
Configure MCP servers in your  Gemini Code settings:
// ~/.config/ Gemini-code/mcp.json
{
  "servers": [
    {
      "name": "email",
...
For sensitive actions,  Gemini writes an approval request file instead of acting directly:
# When  Gemini detects a sensitive action needed:
...
D. Persistence (The "Ralph Wiggum" Loop)
 Gemini Code runs in interactive mode - after processing a prompt, it waits for more input.
...
Orchestrator creates state file with prompt
 Gemini works on task
 Gemini tries to exit
Stop hook checks: Is task file in /Done?
YES → Allow exit (complete)
NO → Block exit, re-inject prompt, and allow  Gemini to see its own previous failed output (loop continues).
...
Promise-based (simple):  Gemini outputs `<promise>TASK_COMPLETE</promise>`
...
The Process:  Gemini Code reads your Business_Goals.md, checks your Tasks/Done folder for the week, and checks your Bank_Transactions.md.
...
Weekly Audit Logic
 Gemini uses pattern matching to identify subscription usage:
# audit_logic.py
...
Logic Engine: Gemini Code (running gemini-3-flash-preview or any other LLM using Gemini Code Router).
...
Gemini Code unavailable: Watchers continue collecting, queue grows for later processing
...
These resources provide a foundational guide on how to integrate Gemini Code agentic capabilities with local tools and file systems, which is the exact "foundation layer" required for your personal employee project.
Gemini Code Chapter of our Textbook
https://agentfactory.panaversity.org/docs/AI-Tool-Landscape/claude-code-features-and-workflows 
Turning Gemini Code into an Employee
https://www.facebook.com/reel/1521210822329090 
Gemini Code and Obsidian for Personal Automation
https://www.youtube.com/watch?v=sCIS05Qt79Y
Gemini Agent Skills - Automate Your Workflow Fast
https://www.youtube.com/watch?v=nbqqnl3JdR0

Gemini Code just Built me an AI Agent Team (Gemini Code + Skills + MCP)
...
Must use Gemini Code as the primary reasoning engine
...
Step 2: Reasoning (Gemini Code)
The Orchestrator triggers Gemini to process the Needs_Action folder:
# Gemini reads the file and creates:
...
Gemini creates an approval request:
...
Step 5: Completion
Gemini updates the Dashboard and moves files to Done:
...
Troubleshooting FAQ
Setup Issues
Q: Gemini Code says "command not found"
A: Ensure Gemini Code is installed globally and your PATH is configured. Run: npm install -g @anthropic/claude-code, then restart your terminal.
Q: Obsidian vault isn't being read by Gemini
A: Check that you're running Gemini Code from the vault directory, or using the --cwd flag to point to it. Verify file permissions allow read access.
...
Q: Gemini is making incorrect decisions
...
A: Check that the server process is running (ps aux | grep mcp). Verify the path in mcp.json is absolute. Check Gemini Code logs for connection errors.
...
Q: What if Gemini tries to pay the wrong person?
